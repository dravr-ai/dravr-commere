...
| Crate | Description |
|-------|-------------|
| `dravr-commere` | Core library with models, dispatch pipeline, scheduling, and Expo Push |
| `dravr-commere-mcp` | MCP server exposing notifications via Model Context Protocol |
| `dravr-commere-server` | Unified REST API + MCP server + CLI binary |
...
cargo clippy -p dravr-commere              # core library
cargo clippy -p dravr-commere-mcp          # MCP server
cargo clippy -p dravr-commere-server       # REST+MCP server
...
All errors MUST use `CommereError` with appropriate variants:
```rust
// GOOD
return Err(CommereError::InvalidInput { field: "age".into(), reason: "must be positive".into() });
return Err(CommereError::InsufficientData { required: 7, actual: 3 });
