...
/// Global shared HTTP client for Expo push API
static PUSH_CLIENT: OnceLock<Client> = OnceLock::new();

/// Get the shared HTTP client for push API calls
fn push_client() -> &'static Client {
    PUSH_CLIENT.get_or_init(|| {
        reqwest::ClientBuilder::new()
            .timeout(Duration::from_secs(DEFAULT_TIMEOUT_SECS))
            .connect_timeout(Duration::from_secs(DEFAULT_CONNECT_TIMEOUT_SECS))
            .build()
            .unwrap_or_else(|e| {
                error!("Failed to build push HTTP client: {e}; falling back to default");
                Client::new()
            })
    })
}
...
impl ExpoPushService {
    ...
    pub async fn send(&self, message: &ExpoPushMessage) -> CommereResult<ExpoPushTicket> {
        let tickets = self.send_batch(slice::from_ref(message)).await?;
        tickets
            .into_iter()
            .next()
            .ok_or_else(|| CommereError::push_delivery("Expo", "Empty response from push API"))
    }
    ...
    pub async fn send_batch(
    ...
    ) -> CommereResult<Vec<ExpoPushTicket>> {
        ...
        for chunk in messages.chunks(MAX_BATCH_SIZE) {
            let response = self
                ...
                .await
                .map_err(|e| {
                    CommereError::push_delivery("Expo", format!("Push request failed: {e}"))
                })?;
            ...
            if !status.is_success() {
                ...
                return Err(CommereError::push_delivery(
                    "Expo",
            ...
            let push_response: ExpoPushResponse = response.json().await.map_err(|e| {
                CommereError::push_delivery("Expo", format!("Failed to parse push response: {e}"))
            })?;
    ...
    pub async fn get_receipts(
        &self,
        ticket_ids: &[String],
    ) -> CommereResult<HashMap<String, ExpoPushReceipt>> {
        ...
        let response = self
            ...
            .await
            .map_err(|e| {
                CommereError::push_delivery("Expo", format!("Receipt request failed: {e}"))
            })?;
        ...
        let receipt_response: ExpoReceiptResponse = response.json().await.map_err(|e| {
            CommereError::push_delivery("Expo", format!("Failed to parse receipt response: {e}"))
        })?;
