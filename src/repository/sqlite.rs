...
use crate::error::{CommereError, CommereResult};
use crate::models::{
    ...
    NotificationPreference, ScheduledNotification, TenantId, UpdateScheduledNotificationParams,
...
fn parse_device_token_row(row: &SqliteRow) -> CommereResult<DeviceToken> {
    ...
    let platform_str: String = row
        .try_get("platform")
        .map_err(|e| CommereError::database(format!("Missing platform: {e}")))?;
    let device_name: Option<String> = row.try_get("device_name").unwrap_or(None);
    let active: i32 = row.try_get("active").unwrap_or(1);
    let created_at_str: String = row
        .try_get("created_at")
        .map_err(|e| CommereError::database(format!("Missing created_at: {e}")))?;
    let updated_at_str: String = row
        .try_get("updated_at")
        .map_err(|e| CommereError::database(format!("Missing updated_at: {e}")))?;
    ...
    Ok(DeviceToken {
        ...
        platform: DevicePlatform::from_str_opt(&platform_str)
            .ok_or_else(|| CommereError::database(format!("Invalid platform: {platform_str}")))?,
        device_name,
        active: active != 0,
...
fn parse_preference_row(row: &SqliteRow) -> CommereResult<NotificationPreference> {
    let id_str: String = row
        .try_get("id")
        .map_err(|e| CommereError::database(format!("Missing id: {e}")))?;
    let user_id_str: String = row
    ...
    Ok(NotificationPreference {
        ...
        category: NotificationCategory::from_str_opt(&category_str)
            .ok_or_else(|| CommereError::database(format!("Invalid category: {category_str}")))?,
        enabled: enabled != 0,
        sub_preferences,
        quiet_hours_start,
        quiet_hours_end,
...
fn parse_notification_row(row: &SqliteRow) -> CommereResult<Notification> {
    let id_str: String = row
        .try_get("id")
        .map_err(|e| CommereError::database(format!("Missing id: {e}")))?;
    let user_id_str: String = row
    ...
    let title: String = row
        .try_get("title")
        .map_err(|e| CommereError::database(format!("Missing title: {e}")))?;
    let body: String = row
        .try_get("body")
...
fn parse_scheduled_notification_row(row: &SqliteRow) -> CommereResult<ScheduledNotification> {
    let id_str: String = row
        .try_get("id")
        .map_err(|e| CommereError::database(format!("Missing id: {e}")))?;
    let user_id_str: String = row
    ...
    let schedule_cron: String = row
        ...
        .map_err(|e| CommereError::database(format!("Missing schedule_cron: {e}")))?;
...
impl NotificationRepository for SqliteNotificationRepository {
    async fn upsert_device_token(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to upsert device token: {e}")))?;
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to fetch device token after upsert: {e}"))
        })?;
    ...
    async fn get_device_tokens(
        ...
        .fetch_all(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to get device tokens: {e}")))?;
    ...
    async fn deactivate_device_token(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to deactivate device token: {e}")))?;
    ...
    async fn get_notification_preferences(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to get notification preferences: {e}"))
        })?;
    ...
    async fn upsert_notification_preference(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to upsert notification preference: {e}"))
        })?;
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to fetch preference after upsert: {e}"))
        })?;
    ...
    async fn create_notification(
        ...
        let row = sqlx::query("SELECT * FROM notifications WHERE id = ?")
            ...
            .await
            .map_err(|e| {
                CommereError::database(format!("Failed to fetch notification after create: {e}"))
            })?;
    ...
    async fn list_notifications(
        ...
        let count_row = count_q
            .fetch_one(&self.pool)
            .await
            .map_err(|e| CommereError::database(format!("Failed to count notifications: {e}")))?;
        ...
        .fetch_one(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to count unread notifications: {e}")))?;
        ...
        let rows = data_q
            .fetch_all(&self.pool)
            .await
            .map_err(|e| CommereError::database(format!("Failed to list notifications: {e}")))?;

        let notifications: Vec<Notification> = rows
            .iter()
            .map(parse_notification_row)
            .collect::<CommereResult<Vec<_>>>()?;
    ...
    async fn mark_notification_read(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to mark notification read: {e}")))?;
    ...
    async fn mark_all_notifications_read(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to mark all notifications read: {e}"))
        })?;
    ...
    async fn delete_notification(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to delete notification: {e}")))?;

        Ok(result.rows_affected() > 0)
    }

    async fn get_unread_count(&self, user_id: Uuid, tenant_id: TenantId) -> CommereResult<i64> {
        let row = sqlx::query(
            "SELECT COUNT(*) as cnt FROM notifications WHERE user_id = ? AND tenant_id = ? AND read_at IS NULL",
        )
        .bind(user_id.to_string())
    ...
    async fn mark_notification_opened(
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| CommereError::database(format!("Failed to mark notification opened: {e}")))?;
    ...
    async fn mark_notification_dismissed(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to mark notification dismissed: {e}"))
        })?;
    ...
    async fn get_notification_analytics(
        ...
        let totals = query.fetch_one(&self.pool).await.map_err(|e| {
            CommereError::database(format!("Failed to get notification analytics: {e}"))
        })?;
        ...
        let avg_row = avg_query
            .fetch_one(&self.pool)
            .await
            .map_err(|e| CommereError::database(format!("Failed to get avg time to open: {e}")))?;
        ...
        let breakdown_rows = breakdown_query.fetch_all(&self.pool).await.map_err(|e| {
            CommereError::database(format!("Failed to get category breakdown: {e}"))
        })?;
    ...
    async fn get_scheduled_notification_by_id(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to get scheduled notification by id: {e}"))
        })?;
    ...
    async fn count_scheduled_notifications(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to count scheduled notifications: {e}"))
        })?;
    ...
    async fn create_scheduled_notification(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to create scheduled notification: {e}"))
        })?;
    ...
    async fn list_scheduled_notifications(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to list scheduled notifications: {e}"))
        })?;
    ...
    async fn delete_scheduled_notification(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to delete scheduled notification: {e}"))
        })?;
    ...
    async fn update_scheduled_notification(
        ...
        let result = query.execute(&self.pool).await.map_err(|e| {
            CommereError::database(format!("Failed to update scheduled notification: {e}"))
        })?;
    ...
    async fn get_due_scheduled_notifications(
        ...
        .await
        .map_err(|e| {
            CommereError::database(format!("Failed to get due scheduled notifications: {e}"))
        })?;
